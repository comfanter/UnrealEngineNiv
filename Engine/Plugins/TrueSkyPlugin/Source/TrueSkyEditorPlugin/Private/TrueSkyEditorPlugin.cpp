...
static HWND GetSWindowHWND(const TSharedPtr<SWindow>& Window)
{
	if ( Window.IsValid() )
	{
		...
		{
			return (HWND)WindowsWindow->GetOSWindowHandle();
		}
	}
	return 0;
}
	...
	if ( DllHandle != nullptr )
	{
		...
		if ( ParentWindow.IsValid() )
		{
			...
			if ( EditorInstance.EditorWindowHWND )
			{
				...
				// Disable the Sequencer's High-dpi awareness because it conflicts with UE 4.18's.
				if(SetHighDPIAware)
					SetHighDPIAware(false);
...
//		trueSkyPlugin.InitRenderingInterface();
 		//void* const Env = (trueSkyPlugin.GetActiveSequence() == TrueSkySequence)? trueSkyPlugin.GetRenderEnvironment() : nullptr;
